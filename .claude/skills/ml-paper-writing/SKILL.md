...
## User-Specific Style: Logic-First LLM Safety Writing

When writing ML/AI papers for Xinyu, especially in LLM safety, prioritize logical relationships above all else. Establish a clear reasoning chain before introducing technical details:

`problem -> risk -> gap -> insight -> method -> evidence -> implication`

Every paragraph must have an explicit logical function. Do not write a paragraph merely to describe background, list related work, or report numbers. Each paragraph should answer one question: why the problem matters, what existing methods miss, what mechanism explains the failure, why the proposed design follows from the observation, or what an experiment proves.

### User-Specific Writing Principles

- Build the paper around a mechanism-level claim, not only a performance improvement.
- Start from a concrete safety or reliability risk in LLMs, MLLMs, or LVLMs.
...
Use this default narrative for large-model safety papers:

1. **Real-world risk**: LLMs, MLLMs, or LVLMs are increasingly deployed, but failures such as jailbreaks, hallucinations, unsafe pruning, visual deception, or unreliable reasoning threaten safe deployment.
2. **Model/system vulnerability**: Identify the concrete failure mode.
3. **Insufficiency of existing methods**: Group prior work into categories and explain why each category fails at the mechanism level.
4. **Key observation/insight**: Present empirical evidence that reveals the underlying mechanism.
5. **Proposed method/framework**: Derive the method from the insight.
6. **Evidence**: Use main results, ablations, robustness, and analysis to validate the mechanism and method.
7. **Implication**: Explain why the result improves safety while preserving utility, efficiency, or general capability.

Use a **threat-driven narrative** for jailbreak, attack, or misuse risks. Use a **failure-driven narrative** for hallucination, unsafe behavior, or reliability failures. Use a **mechanism-driven narrative** when internal layers, tokens, neurons, attention, or representations explain the issue. Use a **benchmark-driven narrative** only when a benchmark reveals a previously hidden safety gap.

### Abstract Template In This Style

Use 6-7 sentences:

1. **Capability + risk**: State the model capability and the safety/reliability risk.
2. **Existing limitation**: State why existing methods are insufficient.
3. **Central question/gap**: Name the mechanism-level question.
4. **Key finding**: State the main empirical observation.
5. **Method**: Introduce the proposed method as motivated by the finding.
6. **Components**: Briefly explain the main components and how they address the mechanism.
7. **Evidence**: Summarize safety, utility, efficiency, or robustness results.

Template:

```text
[Models] have achieved [capability], but remain vulnerable to [risk]. Existing methods [do X], yet [mechanism-level limitation]. To understand/address this, we investigate [central question]. We find that [key observation]. Motivated by this insight, we propose [method], a [training-free/plug-and-play/framework] that [core action]. Specifically, [component 1] addresses [issue 1], while [component 2] addresses [issue 2]. Extensive experiments show that [method] [main safety result] while preserving [utility/efficiency/general capability].
```

### Introduction Template In This Style

**P1: Broader safety risk**  
Start from model capability, then immediately introduce the safety or reliability risk.

```text
Recent advances in [models] have enabled [capability]. However, [failure/risk] remains a critical obstacle for [safe deployment/trustworthy reasoning/reliable multimodal understanding].
```

**P2: Concrete technical problem**  
Define the specific failure mode.

```text
In this work, we focus on [specific problem], where [mechanism/failure behavior] causes [harmful consequence].
```

**P3: Why existing methods are insufficient**  
Group prior methods by paradigm, not paper-by-paper.

```text
Existing methods mainly fall into [category 1], [category 2], and [category 3]. While effective in [setting], they fail to [uncover/control/address] [mechanism-level gap].
```

**P4: Key observation/insight**  
State empirical findings as the bridge from problem to method.

```text
To answer this, we conduct a systematic analysis of [object]. We find that [finding 1], [finding 2], and [finding 3]. These findings indicate that [mechanism] is the key factor behind [failure].
```

**P5: Proposed method**  
Derive the method from the insight.

```text
Motivated by this insight, we propose [method]. The core idea is to [action] by [component mapping].
```

**P6: Experiments and contributions**  
Close the logic loop.

```text
Extensive experiments across [models/benchmarks/settings] demonstrate that [method] [safety improvement] while preserving [utility/efficiency]. In summary, our contributions are: ...
```

### Method Section Guidance

Before describing modules, explain why the method must be designed this way.

Use this component pattern:

`observed failure -> design goal -> module -> mechanism -> expected effect`

Template:

```text
Because [observed problem] causes [failure], we design [module] to [mechanistic function]. Specifically, [module] [operation]. This allows the model to [desired effect] while avoiding [undesired effect].
```

Rules:

- Do not introduce a component without first naming the problem it solves.
- Keep design choices causally tied to empirical observations.
- Use an overview before details.
- For multi-component frameworks, explain why each component is necessary and how components interact.
...
- "Despite strong performance across ..., [models] remain vulnerable to ..."
- "Such failures undermine model reliability and hinder safe deployment."
- "The safety implications of ... remain largely unexplored."

Gap:

- "Existing methods primarily rely on ..., without uncovering ..."
- "Yet, it remains unclear whether ..."
- "The root cause lies in ..."
- "This limitation prevents ... from generalizing to ..."

Insight:

- "To answer this, we conduct a systematic analysis of ..."
- "Surprisingly, we find that ..."
- "This analysis yields a key insight: ..."
...
- "The core idea is to ..."
- "This design mitigates ... while preserving ..."

Evidence:

- "These results confirm ..."
- "Ablation studies further validate ..."
- "This suggests that ... is not simply a byproduct of ..."
...
- vague motivation without a concrete safety failure;
- method-first writing before establishing the mechanism;
- claims without mechanism or evidence;
- result dumping without interpretation;
- over-polished AI prose or marketing-style claims;
- weak transitions between gap, insight, and method;
